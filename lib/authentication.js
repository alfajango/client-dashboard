...
exports.authenticateApiToken = function(req, res, next) {
  console.log("API TOKEN AUTH", req.query);
  if (req.query.api_user && req.query.api_token) {
    User.findOne({ apiUser: req.query.api_user, apiToken: req.query.api_token }, function(err, user) {
      if (user) {
        req.logIn(user, next);
      } else {
        next();
