  ...
  if (data.results && data.results.length > 0) {
    ...
    $.each(data.results, function(i, category) {
      ...
      $.each(category.documents, function(i, doc) {
        ...
        modals += '<div id="redmine-document-' + doc.id + '" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="' + doc.title + '" aria-hidden="true"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button><h3 id="redmine-document-' + doc.id + '-label">' + doc.title + '</h3></div><div class="modal-body">' + doc.description.replace(/(?:\r\n|\r|\n)/g, '<br />') + '</div></div>';
      });
    });
  } else if (data.error) {
